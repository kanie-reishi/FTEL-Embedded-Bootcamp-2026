 ...
 *
 * Thought Process:
 * Custom iterator approaches allow us to check characters one by one without
 * concatenating the strings directly, which saves memory and avoids creating
 * large intermediate strings. We can traverse character by character across
 * different strings in the array.
 *
 * Time Complexity: O(N) where N is the total number of characters in all words.
 * Space Complexity: O(M) where M is the size of the array since it copies the
 * vectors in the iterator. (Can be optimized to O(1) with const references).
 */
...
public:
  ...
  bool hasNext() { return wordIndex != words.size(); }
